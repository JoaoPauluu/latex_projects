...
\recuo É fato que, no anel dos inteiros $\mathbb{Z}$, todo elemento $n$ possui uma única expressão, a menos da ordem, em um produto de números primos. Tal resultado é comumente mostrado em um curso introdutório de Teoria de Números ou de Álgebra. \textcite{kummer1847}, ao lidar com a fatoração única em anéis de inteiros ciclotômicos, introduziu o conceito de número ideal.
%Posteriormente, Richard Dedekind formalizou o conceito de ideal como um conjunto.

Posteriormente, Richard Dedekind formalizou o conceito de ideal como um conjunto. É interessante observar que o próprio termo ``módulo'' já aparece nos escritos de Dedekind sobre teoria dos números, em particular nos Suplementos X e XI da obra \emph{Vorlesungen über Zahlentheorie}(Lições sobre Teoria dos Números) de Dirichlet, organizada e ampliada por Dedekind. Ali, um \emph{Modul} designava um subconjunto de um anel de inteiros algébricos fechado sob soma e diferença, isto é, essencialmente um subgrupo aditivo \parencite{dedekind1871}. Assim, o conceito de módulo que estudamos neste trabalho tem raízes etimológicas e conceituais entrelaçadas com a própria noção de ideal.

Um precursor notável, ainda que anterior à axiomatização dos conceitos, foi o Teorema da Base de Hilbert, segundo o qual todo ideal do anel de polinômios $k[x_1,\dots,x_n]$ é finitamente gerado \parencite{hilbert1890}. Esse resultado é, em essência, um caso particular da condição de cadeia ascendente que é discutida nese trabalho: todo ideal em que toda cadeia ascendente se estabiliza é finitamente gerado.

\textcite{noether}, na tradução da obra ``Teoria de Ideais em Anéis'' feita por Emmy Noether, busca condições para generalizar a fatoração prima em anéis que possuem algumas propriedades semelhantes às de $\mathbb{Z}$. Noether demonstrou que, sob algumas condições, todo ideal de um anel comutativo pode ser decomposto como uma interseção finita de ideais primários, e é única, no sentido que os radicais dos ideais de tal decomposição são os mesmos, independente da decomposição. Os anéis que satisfazem tal condição são conhecidos atualmente como Anéis Noetherianos, em homenagem à matemática.
A propriedade Noetheriana afirma que toda cadeia ascendente de ideais $\mathfrak{p}_{1} \subseteq \mathfrak{p}_{2} \subseteq\dots$ é estacionária, isto é, existe $n$, tal que $\mathfrak{p}_{n+i}=\mathfrak{p}_{i}$, para todo $i \in \mathbb{Z}_{+}$.

De forma semelhante, anéis que satisfazem a condição de cadeia descendente são denominados de anéis Artinianos, em homenagem à Emil Artin, o qual posteriormente, descobriu que os anéis que satisfazem a condição de cadeia descendente generaliza os anéis finitos.

O objetivo dessa pesquisa é estudar as condições de cadeia, isto é, a propriedade Noetheriana, e a Artiniana, a qual é análoga à ascendente, porém considerando cadeias descendentes. Consideramos tais propriedades em objetos mais gerais que anéis: módulos.
...
A pequisa justifica-se pela relevância matemática e histórico-matemática do tema, e pela sua capacidade de generalização de conceitos estudados em Álgebra Linear.

\section*{Materiais e Métodos}

\recuo A pesquisa é de cunho qualitativo-dedutivo, e procura exibir, seguindo o costume matemático, definições apropriadas, demonstrações lógicas e exemplos, que ajudam a ilustrar os conceitos estudados. Baseando-se na obra ``Álgebra Comutativa'' de \textcite{macdonald}, explicitamos as definições em ordem lógica e intuitiva, e evidenciamos os sentidos nos quais os módulos generalizam os espaços vetoriais e quais propriedades surgem ou desaparecem ao remover a não divisibilidade por zero e os elementos inversos dos elementos escalares.

\section*{Resultados e Discussão}

\recuo Começamos por apresentar a definição e alguns exemplos de módulos. Depois, introduzios as condições de cadeia e mostraremos uma classificação dos módulos Noetherianos, seguida por diversas propriedades do mesmo.
...
\begin{examples}
	...
	\begin{enumerate}
		...
		\item Todo grupo é um $\mathbb{Z}$-módulo. Se $n>0$ basta definir $\mu(n,x) = x + x + \dots + x$ $= \sum_{i=1}^{n}x_i$. Se $n<0, \mu(n,x) = -\mu(-n,x)$ e $\mu(0,x)=0$.
	\end{enumerate}
\end{examples}
...
Como é usual na matemática, quando definimos alguma estrutura, queremos alguma forma de identificar quais estruturas possuem propriedades semelhantes. As ferramentas da Álgebra Comutativa para esse fim são os homomorfismos e isomorfismos de módulos.
\begin{definition}
	...
	$f(x+y) = f(x) + f(y), \forall x,y \in M$ ($f$ é um homomorfismo de grupos entre $M$ e $N$) e $f(ax) = af(x), \forall a \in A, x \in M$.

	Note que a composição de dois homomorfismos de $A$-módulos é também um homomorfismo de $A$-módulos.	Se um homomorfismo foi bijetor, é então chamado de isomorfismo.
\end{definition}
...
\begin{definition}
	...
	Como $M$ é comutativo, todo subgrupo de $M$ é normal. Logo, faz sentido considerar o quociente $M/M'= \{ x +M' : x \in M \}$.
\end{definition}
\begin{observation}
	\label{loc:def_3_:_submódulos.quociente_como_um_a:módulo}
	Definindo $a \bar{x} = ax + M' = \overline{ax}$ , com $a \in A, x \in M$ e $x+M' \in M/M'$, temos uma ação linear de $A$ sobre $M/M'$.
	Definindo $\phi:M \to M/M'$ como $\phi(x) = x+M'$ , temos um homomorfismo de $A$-módulos.
\end{observation}
Na Álgebra Linear, existem subconjuntos de espaços vetoriais denominados geradores, e são tais que qualquer elemento do espaço pode ser escrito como combinação linear dos elementos desse conjunto. Mais ainda, tais geradores podem ser reduzidos a bases, as quais possuêm um comprimento único, dependendo apenas do espaço vetorial em questão. Nos módulos, devido a não garantia de existência de inversos para os escalares, tal conceito de base não é bem definido. Não obstante, existe uma noção análoga a de gerador:
\begin{definition}
...
\begin{definition}
	...
	\begin{itemize}
		\item Definimos $\sum_{i \in I} M_{i} = \left\{  \sum x_{i}: x_{i} \in M_{i}, \forall i \text{ e } x_{i}\not= 0 \text{ apenas para finitos i's}  \right\}$. O conjunto $\sum_{i \in I} M_{i}$ é o menor submódulo de $M$ que contém todos os $M_{i}$ (é o conjunto gerado por tais).
		...
		\item A soma direta $\oplus_{i \in I}M_{i} \subseteq \prod_{i \in I} M_{i}$  é o conjunto de todas as funções $f:I \to \cup M_{i},$ tal que $f(i)= x_{i} \in M_{i}, \forall i$ e $f(i) \neq 0$ apenas para um número finito de índices $i$.
	\end{itemize}
\end{definition}
\begin{definition}
...
\begin{observation}
	...
	Com efeito, se $\bar{x} \in A/\mathfrak{a}$ representado por $x \in A$, definimos $\bar{x}m=xm$. Se $\bar{x}=\bar{y}$, temos que $x\equiv y \pmod{\mathfrak{a}}$ implicando que $x-y \in \mathfrak{a} \subseteq Ann(M)$, logo $(x-y)M = 0$ e, em particular, $(x-y)m =0$ implicando que $xm = ym$. Portanto, esta ação está bem definida.
...
\end{observation}
Na Álgebra linear, todo espaço vetorial finitamente gerado é isomorfomo a uma potência do corpo a qual age nesse espaço. Isso nem sempre acontece com módulos, porém, podemos garantir que um módulo finitamente gerado é isomorfo a um quociente dessa potência, conforme feito no próximo resultado.
\begin{proposition}
...
\recuo Nesta seção, introduzimos o conceito central da pesquisa: cadeias ascendentes e descendentes. Primeiramente, retomemos uma definição e um resultado da teoria de conjuntos, os quais serão convenientes nas discussões adiante.
\begin{definition}
...
Para entender a estrutura de um módulo complicado, comumente buscamos reduzi-lo em submódulos e módulos quocientes menores. A ferramenta algébrica para estudar essas relações estruturais e a preservação de propriedades (como ser Noetheriano ou Artiniano) é a sequência exata:
\begin{definition}
...
\end{definition}
O próximo resultado evidência a importância das sequências exatas no estudo dos módulo ao afirmar que as sequências exatas curtas preservam a propriedade de um módulo ser ou não Noetheriano e Artiniano.
\begin{proposition}
	...
	\begin{enumerate}
		\item $M$ Noetheriano se, e somente se, $M'$ e $M''$ são Noetherianos;
		\item $M$ Artiniano se, e somente se, $M'$ e $M''$ são Artinianos.
	\end{enumerate}
	\begin{proof}
...
\begin{corollary}
	...
	\end{proof}
\end{corollary}
O próximo resultado afirma que podemos carregar certas propriedades de um anel para os módulos sobre os quais tal anel atua, no sentido que, se um anel é Noetheriano ou Artiniano, então podemos verificar se algum $A$-módulo é Noetheriano ou Artiniano apenas verificando se este é finitamente gerado.
\begin{proposition}
	...
	\begin{proof}
		...
		Como $M$ é finitamente gerado, pelo \Cref{loc:2.3_:_equivalência_de_finitamente_gerado.}, $M$ é isomorfo a $\frac{A^{n}}{Q}$ para algum $Q$ ideal de $A^{n}$.
		Note que
		\begin{equation*}
			\begin{CD}
...
\begin{proposition}
	...
	\begin{proof}
		Conforme feito na \Cref{loc:6.5_:_anéis_noetherianos_e_artinianos_e_módulos}, temos que $A/\mathfrak{a}$ é Noetheriano (respectivamente Artiniano) como um $A$-módulo.
	...
	\begin{proof}
		...
		\begin{enumerate}
			...
			\item Toda série de composição de $M$ tem comprimento igual a $l(M)$ e toda cadeia pode se tornar uma série de composição.
			      Seja $(M_{i})$ uma série de composição de $M$ de comprimento $k$. Temos $l(M)\leq k$ pela definição.
...
\begin{definition}
	...
	Baseando-se no \Cref{loc:6.8_:_série_de_composição_e_condições_de_cadeia.proposição}, se $M$ satisfaz a.c.c e d.c.c então $M$ é dito de comprimento finito.
\end{definition}
