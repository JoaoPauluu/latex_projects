...
\begin{definition}
    ...
    \begin{figure}[htbp]
        \centering
        \caption{Diagrama de parametrização de uma variedade diferenciável.}

        \input{tikzpictures/parametrizations_diagram.tex}
...
\begin{lemma}
    \label{loc:!resultados_extras_importantes!identificar_espaco_tangente}
...
\begin{figure}[htbp]
    \centering
    \caption{Diagrama do fluxo local de um campo vetorial $X$ em uma variedade diferenciável $M$.}

    \input{tikzpictures/local_flux.tex}

    \vspace{6pt}
    \small Fonte: Elaborado pelo autor (2026).
    \label{fig:fluxo_local}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \caption{Diagrama do operador colchetes.}

    \input{tikzpictures/lie_bracket.tex}
...
\begin{theorem}
    \label{loc:2.10_:_teo_3_:_condição_para_partição_da_unidade}
    Uma variedade diferenciável $M$ possui partição diferenciável da unidade se, e somente se, toda compontente conexa de $M$ é Hausodorff e tem base enumerável.

    \begin{proof}
        Consultar \textbf{PLACEHOLDER: INCERIR REFERÊNCIA}
    \end{proof}
\end{theorem}
