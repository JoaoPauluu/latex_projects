...
\section*{Introdução}
É fato que, no anel dos inteiros $\mathbb{Z}$, todo elemento $n$ possuí uma única expressão, a menos da ordem, em números primos. Tal resultado é comumente mostrado em um curso introdutório de Teoria de Números ou de Álgebra. Emmy Noether, em seu artigo "Teoria de Ideais em Anéis", publicado no ano de 1921, (https://arxiv.org/pdf/1401.2577) busca condições para generalizar tal fatoração prima em anéis que possuem algumas propriedades semelhantes as de $\mathbb{Z}$. E. Noether demonstrou que, sob algumas condições, todo ideal de um anel comutativo pode ser decomposto como uma interseção finita de Ideais primários, e é única, no sentido que os radicais dos ideais de tal decomposição são os mesmos, independente da decomposição. Os anéis que satisfazem tal condição são conhecidos hoje em dia como Anéis Noetherianos, em homenagem a matemática.
...
O objetivo dessa pesquisa é estudar as condições de cadeia, isto é, a propriedade Noetheriana, e a Artiniana, a qual é análoga a ascendente, porém considerando cadeias descendentes. Como de costume, consideraremos tais propriedades em objetos mais gerais que anéis: módulos.
...
O objetivo dessa pesquisa é estudar as propriedades dos módulos noetherianos e artinianos e comparar tais módulos com os espaços vetoriais finitamente gerados. Ela justifica-se pela relevância matemática e histórico-matemática do tema, e pela capacidade de generalização.

\section*{Materiais e método}
A pesquisa é de cunho qualitativo-dedutivo, e procura exibir, seguindo o costume matemático, definições apropriadas, demonstrações lógicas e exemplos, que ajudam a ilustrar os conceitos estudados. Baseando-se na obra "Álgebra Comutativa" de Atiyah e McDonald (CITAR O MACDONALDS), explicitaremos as definições em ordem lógica e intuitiva, e evidenciaremos os sentidos nos quais os módulos generalizam os espaços vetoriais e quais propriedades mudam ao remover a não divisibilidade por zero e os elementos inversos.
...
\subsection*{Módulos}
\label{loc:módulos}
Nesse texto, todos os anéis são comutativos e com unidade, salvo menção contrária.
...
Nesta sessão, introduziremos o conceito central da pesquisa: Cadeias ascendentes e descendentes. Primeiramente, retomemos uma definição da teoria de conjuntos, a qual será conveniente nas definições.
\begin{definition}
